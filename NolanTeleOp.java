...
public class NolanTeleOp extends OpMode {
    ...
    public void init() {
        ...
        right_servo.setPosition(0.55); // open
...
//      double volts = hardwareMap.voltageSensor.get("Motor Controller 1").getVoltage();
    ...
    public void loop() {
        ...
        float inputGripperOpen = gamepad2.left_trigger;
        float inputGripperClose = gamepad2.right_trigger;

        if (inputGripperClose > 0.5) gripper_is_open = false;
        if (inputGripperOpen > 0.5) gripper_is_open = true;

        arcadeMecanum(inputY, inputX, inputC, left_front_wheel, right_front_wheel, left_back_wheel, right_back_wheel, inputLift, lift_motor, left_servo, right_servo);
    }

    // y - forwards
    // x - side
    // c - rotation
    public static void arcadeMecanum(double y, double x, double c, DcMotor leftFront, DcMotor rightFront, DcMotor leftBack, DcMotor rightBack, double inputLift, DcMotor lift_motor, Servo left_servo, Servo right_servo) {
        ...
        if (gripper_is_open) {
            left_servo.setPosition(.4);
            right_servo.setPosition(.55);
        } else {
            left_servo.setPosition(.1);
            right_servo.setPosition(.9);
        }
    }
}
