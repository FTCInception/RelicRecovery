...
@Autonomous(name="RED_Position_FAR_No_Jewel", group="RED_FAR")

public class RED_Autonomous_FAR_No_Jewel extends LinearOpMode {
    ...
    public void runOpMode()
    {
        ...
        /*if (Math.abs(245-sensorColor.red())<=colorTolerance && opModeIsActive())
        {
            //the color of the ball facing the sensor is red
            //we need to move forwards
            movement(.25, .25, .25, .25, eTime,  .25);
            telemetry.addData("Color found is","red");//tells us if we actually saw red - Peter
            sleep(2000);


        }
        else if (Math.abs(245-sensorColor.blue())<=colorTolerance && opModeIsActive())
        {
            //the color of the ball facing the sensor is blue
            //we need to move to backwards
            movement(-.25, -0.25, -.25, -.25, eTime, .25);
            telemetry.addData("Color found is","blue");//tells us if we actually saw blue - Peter
            sleep(2000);
